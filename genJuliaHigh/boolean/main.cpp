...
void writeJulia(string ID, string path, string ExpNum, string senID, string FeatureNum, string nlevel)
{
    ofstream juliaFile;
    juliaFile.open(path+"level.jl");
    juliaFile << "using Pkg\nusing JuMP\nusing Gurobi\nusing DataFrames, CSV\n";
    juliaFile << "\n";
    juliaFile << "m = Model(Gurobi.Optimizer)\n";
    juliaFile << "set_optimizer_attribute(m, \"NonConvex\", 2)\n";
    juliaFile << "\n";
    juliaFile << "# set macro, the size of outputClass, input features, .. etc.\n";
    juliaFile << "NE = " + ExpNum + "; # number of examples\n";
    juliaFile << "NF = " + FeatureNum + "; # number of input features\n";
    ...
    juliaFile << "@constraint(m, zitDef2[i=1:NE], Zit[i, 1] == 1)\n";
    juliaFile << "\n";
    juliaFile << "def1 = JuMP.Containers.DenseAxisArray{ConstraintRef}(undef, 1:NL, 1:NO);\n";
    ...
    juliaFile << "sensitiveFeature = JuMP.Containers.DenseAxisArray{ConstraintRef}(undef, 1:NB);\n";
    ...
    // bobo add "Int" to make the Bm range in [0,1]
    ...
    juliaFile << "\n";
    juliaFile << "branch = JuMP.Containers.DenseAxisArray{ConstraintRef}(undef, 1:(NL+NB), 1:nlevel, 1:NE);\n";
    ...
    juliaFile << "horizontalSum = JuMP.Containers.DenseAxisArray{ConstraintRef}(undef, 1:nlevel, 1:NE)\n";
    ...
    juliaFile << "sumChildren = Vector{ConstraintRef}(undef, ((2^nlevel)-1))\n";
    ...
    juliaFile << "levelDep = JuMP.Containers.DenseAxisArray{ConstraintRef}(undef, 1:numNeed, 1:2)\n";
    ...
    juliaFile << "zeroAmBm = JuMP.Containers.DenseAxisArray{ConstraintRef}(undef, 1:(2^nlevel-1), 1:NE)\n";
    juliaFile << "for i = 1:(2^nlevel-1)\n";
    juliaFile << "\tfor j = 1:NE\n";
    ...
    juliaFile << "Dataset = CSV.read(joinpath(dirname(pathof(DataFrames)), dataPath*\"test$dataID.csv\"))\n";
    ...
    juliaFile << "\n";
    juliaFile << "FPt_cons = JuMP.Containers.DenseAxisArray{ConstraintRef}(undef, 1:NE, 1:NL)\n";
    juliaFile << "for i = 1:NE\n";
    ...
    juliaFile << "end\n";
    juliaFile << "\n";
    juliaFile << "Ft_cons = JuMP.Containers.DenseAxisArray{ConstraintRef}(undef, 1:NE, 1:NL)\n";
    juliaFile << "for i = 1:NE\n";
    juliaFile << "\tfor j = 1:NL\n";
    ...
    juliaFile << "end\n";
    juliaFile << "\n";
    juliaFile << "MPt_cons = JuMP.Containers.DenseAxisArray{ConstraintRef}(undef, 1:NE, 1:NL)\n";
    juliaFile << "for i = 1:NE\n";
    juliaFile << "\tfor j = 1:NL\n";
    ...
    juliaFile << "end\n";
    juliaFile << "\n";
    juliaFile << "Mt_cons = JuMP.Containers.DenseAxisArray{ConstraintRef}(undef, 1:NE, 1:NL)\n";
    juliaFile << "for i = 1:NE\n";
    juliaFile << "\tfor j = 1:NL\n";
    ...
    juliaFile << "optimize!(m)\n";
    juliaFile << "\n";
    juliaFile << "io = open(dataPath*\"res.txt\", \"w\")\n";
    //juliaFile << "io = open(\"/Users/bobobo/Documents/fairSyn/juliaCode/testData/res.txt\", \"w\")\n";
    // bobo for higher version, emit getvalue() function
    juliaFile << "println(io, dataID)\n";
    juliaFile << "println(io, JuMP.value.(Bm[1]))\n";
    juliaFile << "for i = 1:NF\n";
    juliaFile << "\tif JuMP.value.(Am[i,1]) != 0.0\n";
    juliaFile << "\t\tprintln(io, i)\n";
    juliaFile << "\tend\n";
    juliaFile << "end\n";
    juliaFile << "println(io,0)\n";
    juliaFile << "close(io)\n";
    juliaFile << "\n";
    juliaFile << "println(\"Final Solution: [ SumFPt : $(JuMP.value.(SumFPt))]\")\n";
    juliaFile << "println(\"Final Solution: [ SumDt: $(JuMP.value.(sumDt))]\")\n";
    juliaFile << "println(\"Final Solution: [ fairVar : $(JuMP.value.(fairVar))]\")\n";
    juliaFile << "println(\"Final Solution: [ fair : $(value(fair))]\")\n";
    juliaFile << "println(\"Final Solution: [ sumLt : $(value(sumLt))]\")\n";
    juliaFile << "println(\"Final Solution: [ Yik : $(JuMP.value.(Yik))]\")\n";
    juliaFile << "println(\"Final Solution: [ Nt : $(JuMP.value.(Nt))]\")\n";
    juliaFile << "println(\"Final Solution: [ Lt : $(JuMP.value.(Lt))]\")\n";
    juliaFile << "println(\"Final Solution: [ Zit : $(JuMP.value.(Zit))]\")\n";
    juliaFile << "println(\"Final Solution: [ Am : $(JuMP.value.(Am))]\")\n";
    juliaFile << "println(\"Final Solution: [ Bm : $(JuMP.value.(Bm))]\")\n";
    juliaFile.close();
}
...
int main(int argc, const char * argv[]) {
    ...
    if(argc < 7)
    {
        cout << "Warning : number of arguments is less than 7!" << endl;
        return 0;
    }
    ...
    // argv[5] : the number of available features in the dataset
    // argv[6] : the nlevel deciding the level architecture in the MIP encoding
    writeJulia(argv[1], dataPath, argv[3], argv[4], argv[5], argv[6]);
